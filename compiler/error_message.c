...
ErrorDefinition dkc_error_message_format[] = {
    ...
    {"ひとつのソースから同一のパッケージを複数回requireしています($(package_name))。"},
    ...
    {"Disamでは、abstractクラス以外は継承できません(クラス$(name)はabstractではありません)。"},
    {"abstractクラス($(name))をnewしています。"},
    {"void型の関数が値を返しています。"},
    {"クラス$(name)が見付かりません。"},
    {"コンストラクタとして指定されているメンバ$(member_name)はメソッドではありません。"},
    {"newで指定されているメソッド$(member_name)がコンストラクタではありません。"},
    ...
    {"このinstanceofは常に真です。"},
    {"このinstanceofは常に偽です。"},
    {"Diksamのインターフェイスには親子関係はないので、このinstanceofは常に偽です。"},
    {"ダウンキャストの対象となるのはクラスだけです。"},
    {"ダウンキャストの変換先となるのはクラスだけです。"},
    {"このダウンキャストは不要です。"},
    {"スーパークラスにダウンキャストしようとしています。"},
    {"継承関係のないクラスにダウンキャストしようとしています。"},
    {"Diksamのインターフェイスには親子関係はないので、ダウンキャストできません。"},
    {"クラス以外の型をcatchしています。"},
    {"catchしているクラス$(class_name)はExceptionのサブクラスではありません。"},
    {"クラス以外の型をthrowしています。"},
    {"throwしているクラス$(class_name)はExceptionのサブクラスではありません。"},
    ...
    {"例外$(name)はthrows節に指定されるか、内部でcatchされなければなりません。"},
    {"自分自身をrequireしています。"},
    ...
    {"オーバーライドしているメソッド$(name)のアクセス修飾子が、スーパークラスに比べ狭くなっています。"},
    {"メソッドまたは関数$(name)の引数の数が間違っています。"},
    {"メソッドまたは関数$(func_name)の$(index)番目の引数$(param_name)の型が間違っています。"},
    {"メソッドまたは関数$(name)の戻り値の型が不正です。"},
    {"メソッドまたは関数$(func_name)が、適合しない例外$(exception_name)をthrowsに記述しています。"},
